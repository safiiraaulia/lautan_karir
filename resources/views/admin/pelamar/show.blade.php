        ...
        <div class="col-md-3">
                ...
                <div class="card-body box-profile text-center">
                    ...
                    <ul class="list-group list-group-unbordered mb-3 text-start">
                        <li class="list-group-item d-flex justify-content-between">
                            <b>Status</b>
                            <span class="badge {{ $pelamar->is_active ? 'bg-success' : 'bg-danger' }}">
                                {{ $pelamar->is_active ? 'Aktif' : 'Non-Aktif' }}
                            </span>
                        </li>
                    </ul>
                </div>
            ...
            <div class="card shadow-sm mt-3 no-print">
                <div class="card-header bg-dark text-white py-2">
                    <h6 class="card-title mb-0"><i class="fas fa-file-alt mr-1"></i> Berkas & Dokumen</h6>
                </div>
                <ul class="list-group list-group-flush">
                    ...
                    @foreach(['CV' => 'path_cv', 'KTP' => 'path_ktp', 'Ijazah' => 'path_ijazah', 'KK' => 'path_kk', 'Lamaran' => 'path_lamaran'] as $label => $field)
                        <li class="list-group-item d-flex justify-content-between align-items-center py-2">
                            {{ $label }}
                            @if($pelamar->$field)
                                <a href="{{ Storage::url($pelamar->$field) }}" target="_blank" class="btn btn-sm btn-primary py-0 px-2" title="Lihat">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center py-2">
                        ...
                        @if($pelamar->path_transkrip)
                            <a href="{{ Storage::url($pelamar->path_transkrip) }}" target="_blank" class="btn btn-sm btn-primary py-0 px-2" title="Lihat">
                                <i class="fas fa-eye small"></i>
                            </a>
                        @else
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <dl class="row mb-0">
                                <dt class="col-sm-4 text-muted">NIK (KTP)</dt>
                                <dd class="col-sm-8 fw-bold">{{ $pelamar->no_ktp ?? '-' }}</dd>

                                <dt class="col-sm-4 text-muted">Tempat Lahir</dt>
                                <dd class="col-sm-8">{{ $pelamar->tempat_lahir ?? '-' }}</dd>
                                ...
                                <dd class="col-sm-8">{{ $pelamar->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</dd>

                                {{-- TAMBAHAN: Agama --}}
                                <dt class="col-sm-4 text-muted">Agama</dt>
                                <dd class="col-sm-8">{{ $pelamar->agama ?? '-' }}</dd>

                                <dt class="col-sm-4 text-muted">Status Nikah</dt>
                                <dd class="col-sm-8">{{ $pelamar->status_pernikahan ?? '-' }}</dd>

                                <dt class="col-sm-4 text-muted">Warga Negara</dt>
                                <dd class="col-sm-8">{{ $pelamar->kewarganegaraan ?? 'WNI' }}</dd>
                            </dl>
                        </div>
                        <div class="col-md-6">
                            <dl class="row mb-0">
                                <dt class="col-sm-4 text-muted">Tinggi/Berat</dt>
                                <dd class="col-sm-8">{{ $pelamar->tinggi_badan ?? '-' }} cm / {{ $pelamar->berat_badan ?? '-' }} kg</dd>

                                <dt class="col-sm-4 text-muted">Gol. Darah</dt>
                                <dd class="col-sm-8">{{ $pelamar->golongan_darah ?? '-' }}</dd>

                                <dt class="col-sm-4 text-muted">Alamat</dt>
                                <dd class="col-sm-8">{{ $pelamar->alamat_domisili ?? '-' }}</dd>

                                <dt class="col-sm-4 text-muted">Tempat Tinggal</dt>
                                <dd class="col-sm-8">{{ $pelamar->status_tempat_tinggal ?? '-' }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                @forelse($pelamar->pekerjaan as $job)
                                    <tr>
                                        ...
                                        <td>{{ $job->nomor_atasan ?? '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted py-3">Tidak ada data pengalaman kerja.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="row g-3">
                        @foreach(['NPWP' => 'no_npwp', 'BPJS TK' => 'no_bpjs_tk', 'BPJS Kes' => 'no_bpjs_kes', 'SIM A' => 'no_sim_a', 'SIM C' => 'no_sim_c'] as $lbl => $fld)
                            <div class="col-md-4">
                                ...
                                <strong>{{ $pelamar->$fld ?? '-' }}</strong>
                            </div>
                        @endforeach
                        <div class="col-md-4">
                            <small class="text-muted d-block">Kendaraan</small>
                            <strong>{{ $pelamar->jenis_kendaraan ?? '-' }} ({{ $pelamar->kepemilikan_kendaraan ?? '-' }})</strong>
                        </div>
                        <div class="col-md-4">
                            <small class="text-muted d-block">Merk/Tahun</small>
                            <strong>{{ $pelamar->merk_kendaraan ?? '-' }}</strong>
                        </div>
                    </div>
                </div>
            </div>
